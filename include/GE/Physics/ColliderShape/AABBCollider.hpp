...
namespace Engine::Physics::ColliderShape
{
    ...
    public:
        ...
        { 
            const Engine::Core::Maths::Vec3 centerGlobal = gameObject.getPosition() + AABB_.getCenter();
            const float extIGlobal = AABB_.getExtI() + gameObject.getScale().x / 2.f;
            const float extJGlobal = AABB_.getExtJ() + gameObject.getScale().y / 2.f;
            const float extKGlobal = AABB_.getExtK() + gameObject.getScale().z / 2.f;
