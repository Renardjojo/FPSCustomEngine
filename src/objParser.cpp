...
void parseIndice (const std::string& line, std::vector<Shape>& shape)
{
    ...
    if(numberIndice == 0)
    {
        SLog::logWarning((std::string("Indice without vn not accept. Line not read : ") + line).c_str());
        return;
    }
...
void parseLine (const std::string& line, Attrib* attrib, std::vector<Shape>* shape, std::vector<MaterialAttrib>* materials)
{
    ...
    if      (isTitle(line, "#"))                                return;
                                                                ...
                                                                SLog::log((std::string("Obj name : ") + attrib->objName).c_str()); }
    ...
    else                                                        SLog::logWarning(std::string("Obj Parser don't parse this line : " + line).c_str());
}
...
{
    GE_assert(objPath != nullptr);

    SLog::logInitializationStart("Obj parsing");

    std::ifstream file(objPath);

	if(!file)
	{
        SLog::logError((std::string("Error to found or read file : ") + objPath).c_str());
        exit(1);
	}
