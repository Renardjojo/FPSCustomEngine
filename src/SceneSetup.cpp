...
void saveScene(Scene &scene, GE &gameEngine, const char *filePath)
{
    xml_document<> doc;
    ...
    for (GameObject& gameObjectParent : scene.getWorld().children)
        saveEntity(gameObjectParent, gameEngine, doc, worldNode);
...
void saveEntity(GameObject& gameObjectParent, GE& gameEngine, xml_document<>& doc, xml_node<>* nodeParent)
{
    ...
    for (GameObject &gameObjectParent : gameObjectParent.children)
        saveEntity(gameObjectParent, gameEngine, doc, newNode);
